...
public class QuotationServiceImpl implements QuotationService {
    ...
    public QuotationEntity savingQuo(QuotationDto quotationDto, String id) {
        ...
        String regionSplit = quotationDto.getGosuRegion().substring(1, quotationDto.getGosuRegion().length()-1);
...
//
    ...
    @Override
    public void statusFinish(String quotationId) {
        try{

            QuotationEntity target = quotationRepo.findById(quotationId).orElseThrow();
            QuotationDto result = target.toDto();
            result.setStatus(2);

            quotationRepo.save(result.toEntity());
